...
function formatDateForAxis(date, unit) {
    ...
    if (unit === 'day') {
        const result = `${day} ${shortMonth} ${year}`;
...
class PriceAnalyzer {
     ...
     */
    async loadProductData(categoryId, productName, searchValue = null, dateFrom = null, dateTo = null, searchMode = 'exact') {
		console.log('Загрузка данных товара:', { categoryId, productName, searchValue, searchMode });
		...
		try {
			...
			if (searchValue && searchValue.trim()) {
				if (searchMode === 'keywords') {
					url += `&keywords=${encodeURIComponent(searchValue)}`;
				} else {
					url += `&characteristic=${encodeURIComponent(searchValue)}`;
				}
			}
    ...
    /**
     * Подготовка данных для линейного графика (ИСПРАВЛЕННАЯ ВЕРСИЯ)
     */
    ...
    /**
     * Подготовка данных для пузырьковой диаграммы (ИСПРАВЛЕННАЯ ВЕРСИЯ)
     */
                    ...
                    x: {
                        ...
                        ticks: {
                            ...
                            callback: function(value, index, values) {
                                ...
                                return formatDateForAxis(date, timeUnit);
                            }
                        }
                    }
                },
	...
	/**
	 * Подготовка данных для линейного графика по стоимости покупок (amount)
	 */
	prepareLineChartDataByAmount(purchases) {
		if (!purchases || purchases.length < 2) {
			return null;
		}
		...
		// Берём сумму покупки (amount) вместо цены за единицу
		const data = sorted.map(p => parseFloat(p.amount) || 0);
				...
				plugins: {
					tooltip: {
						callbacks: {
							...
							label: function(context) {
								return `💰 ${ChartUtils.formatCurrency(context.raw)}`;
							}
						}
					}
				}
...
console.log('PriceAnalyzer загружен и доступен через window.priceAnalyzer');